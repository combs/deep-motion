...
from keras.layers import Layer, Input, MaxPooling2D, UpSampling2D, merge, BatchNormalization, Activation
from keras.layers.convolutional import Conv2D as Convolution2D
from keras.layers.convolutional import Deconv2D as Deconvolution2D
...
def conv_block(input, nb_filter):
    conv1 = Convolution2D(nb_filter, nb_row=3, nb_col=3, padding='same')(input)
    relu1 = Activation("relu")(conv1)
    conv2 = Convolution2D(nb_filter, nb_row=3, nb_col=3, padding='same')(relu1)
    relu2 = Activation("relu")(conv2)
    conv3 = Convolution2D(nb_filter, nb_row=3, nb_col=3, padding='same')(relu2)
...
def get_unet_bak():
    ...
    # conv5 = Convolution2D(256, 3, 3, activation='relu', padding='same')(pool4)
    # conv5 = Convolution2D(256, 3, 3, activation='relu', padding='same')(conv5)
    #
    # up6 = merge([UpSampling2D(size=(2, 2))(conv5), conv4], mode='concat', concat_axis=1)
    conv6 = Convolution2D(128, 3, 3, activation='relu', padding='same')(pool4)
    ...
    conv8 = Convolution2D(32, 3, 3, activation='relu', padding='same')(up8)
    ...
    conv9 = Convolution2D(16, 3, 3, activation='relu', padding='same')(up9)
